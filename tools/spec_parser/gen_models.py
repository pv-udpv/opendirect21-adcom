        ...
        if not obj_def.fields:
            ...
            for field in obj_def.fields:
                ...
                if py_type in [o.name for o in [obj_def]]:  # Self-reference
                    py_type = f'"{py_type}"'
                elif not py_type.startswith(
                    ("str", "int", "float", "bool", "datetime", "date", "List", "Dict")
                ):
                ...
                if field_name != original_name:
                    ...
                    lines.append(f"    {field_name}: {full_type}{default}  # {desc}")

        return "\n".join(lines)

    def generate_all_models(
        self, objects: list[ObjectDef], output_file: str = "opendirect.py"
    ) -> str:
